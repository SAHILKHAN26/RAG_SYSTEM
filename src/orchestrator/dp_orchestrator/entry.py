...
from src.orchestrator.dp_orchestrator.orchestrator import (
    OrchestratorAgent,
    OrchestratorTaskManager
)
from src.core.constant import DP_ORCHESTRATOR_HOST,DP_ORCHESTRATOR_PORT, DP_ORCHESTRATOR_REGISTRY_FILE


def main(registry: str = DP_ORCHESTRATOR_REGISTRY_FILE, host: str = DP_ORCHESTRATOR_HOST, port: int = DP_ORCHESTRATOR_PORT):
    """
    ...
    skill = AgentSkill(
        id="dp_orchestrate",                          # Unique skill identifier
        name="Orchestrate Tasks for DPAgent",                  # Human-friendly name
        description=(
    ...
    orchestrator_card = AgentCard(
        ...
        url=f"http://{DP_ORCHESTRATOR_HOST}:{DP_ORCHESTRATOR_PORT}/",             # Public endpoint
        version="1.0.0",
