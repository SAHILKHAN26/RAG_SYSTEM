...
# -----------------------------------------------------------------------------
from src.orchestrator.dp_orchestrator.agent_connect import AgentConnector
...
class OrchestratorAgent:
    ...
    def _root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return (
           "You are an orchestrator with multiple agents:\n"
            "1) list_agents() -> list available child agents\n"
            "2) delegate_task(agent_name, message) -> call that agent\n\n"
            
            f"Available agents:\n{agent_list}\n\n"
            "Use NLPAgent when the passed query seems like a article or news and it contains the incident information related to any data breach related to any company"
            "Use RiskAgent when the query is related to calculating the risk of the company"
