...
# -----------------------------------------------------------------------------
from src.orchestrator.graph_rag_orchestrator.agent_connect import AgentConnector
...
class OrchestratorAgent:
    ...
    def _root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return (
           "You are an data privacy orchestrator with multiple agents your task is to use these agents and solve the query:\n"
            "1) list_agents() -> list available child agents\n"
            "2) delegate_task(agent_name, message) -> call that agent\n\n"
            
            f"Available agents:\n{agent_list}\n\n"
            "Use VectorSearch when the passed query seems like a article or news and it contains the incident information related to any data breach related to any company"
            "Use NLP2Cypher when the query is related to asking any incident details or and company related details"
            "Use SuggestionAgent when the user wants to get some suggestion about any company or any incident then only reply"        
